...
public class Signup1 extends JFrame implements ActionListener {
	...
	Signup1() {
		...
		l1.setBounds(140, 35, 600, 40);
		add(l1);

		l2 = new JLabel("Page 1: Personal Details");
		l2.setFont(new Font("Raleway", Font.BOLD, 22));
		l2.setBounds(290, 85, 600, 30);
		add(l2);
		...
		dateChooser.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
		add(dateChooser);
		...
		t5.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
		add(t5);
		...
		t6.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
		add(t6);
		...
		t7.setBorder(BorderFactory.createLineBorder(Color.GRAY, 1));
		add(t7);
	...
	public void actionPerformed(ActionEvent event) {
		...
		try {
			...
			if (t6.getText().equals("")) {   // Nếu pincode trống → báo lỗi
				JOptionPane.showMessageDialog(null, "Fill all the required fields");
			} else {
				Conn c1 = new Conn();
				// Tạo cmd query thêm dữ liệu vào DB
				String q1 = "insert into signup1 values('" + formno + "','" + name + "','" + fname + "','" + dob + "','"
						+ gender + "','" + email + "','" + marital + "','" + address + "','" + city + "','" + pincode
						+ "','" + state + "')";
				
				c1.s.executeUpdate(q1);    // Chạy cmd INSERT

				new Signup2(first).setVisible(true);  // Nhấn Next mở Signup2
				setVisible(false);
			}
